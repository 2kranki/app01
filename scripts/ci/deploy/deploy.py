...
class Main(util.MainBase):
    ...
    def exec_pgm(self):
        ...
        result = self.do_cmd("./scripts/ci/deploy/deploy.py", 'app01ma')
        if result != 0:
            print("Error: Deploying failed for: 'app01ma'!\n")
            return
        result = self.do_cmd("./scripts/ci/deploy/deploy.py", 'app01ms')
        if result != 0:
            print("Error: Deploying failed for: 'app01ms'!\n")
            return
        result = self.do_cmd("./scripts/ci/deploy/deploy.py", 'app01my')
        if result != 0:
            print("Error: Deploying failed for: 'app01my'!\n")
            return
        result = self.do_cmd("./scripts/ci/deploy/deploy.py", 'app01pg')
        if result != 0:
            print("Error: Deploying failed for: 'app01pg'!\n")
            return
        result = self.do_cmd("./scripts/ci/deploy/deploy.py", 'app01sq')
        if result != 0:
