...
type App01pgCustomerDbRowDeleter interface {
    // RowDelete deletes the row with keys from the provided record, rcd.
    RowDelete(rcd *App01pgCustomer) error
}

type App01pgCustomerDbRowFinder interface {
    ...
    RowFind(rcd *App01pgCustomer) error
}

type App01pgCustomerDbRowFirster interface {
    ...
    // without error.
    RowFirst(rcd *App01pgCustomer) error
}

type App01pgCustomerDbRowInserter interface {
    RowInsert(rcd *App01pgCustomer) error
}

type App01pgCustomerDbRowLaster interface {
    // RowLast returns the last row in the table, Customer.
    // If there are no rows in the table, then a blank/null record is returned
    // without error.
    RowLast(rcd *App01pgCustomer) error
}

type App01pgCustomerDbRowNexter interface {
    ...
    RowNext(rcd *App01pgCustomer) error
}

type App01pgCustomerDbRowPager interface {
    ...
    RowPage(offset int, limit int) ([]App01pgCustomer, error)
}

type App01pgCustomerDbRowPrever interface {
    RowPrev(rcd *App01pgCustomer) error
}

type App01pgCustomerDbRowUpdater interface {
    RowUpdate(rcd *App01pgCustomer) error
}

type App01pgCustomerDbTableCounter interface {
    TableCount() (int, error)
}

type App01pgCustomerDbTableCreater interface {
    TableCreate() error
}

type App01pgCustomerDbTableDeleter interface {
    TableDelete() error
}

type App01pgCustomerDbTableScanner interface {
    // TableScan reads all the rows in the table applying a function to each of
    // them.
    TableScan(apply func (rcd App01pgCustomer) error) error
}
...
type App01pgCustomers []*App01pgCustomer

type Key struct {
	Num	int64
}

type App01pgCustomerIndex map[Key]*App01pgCustomer
...
func (s *App01pgCustomer) Compare(r *App01pgCustomer) int {
...
func (s *App01pgCustomer) CompareKeys(r *App01pgCustomer) int {
    // Accumulate the key value(s) in KeyNum order.
    // Field: Num
            if s.Num != r.Num {
                if s.Num < r.Num {
...
func (s *App01pgCustomer) FieldsToValue() string {
    var wrk string
...
func (d *App01pgCustomer) JsonMarshal() ([]byte, error) {
	var err         error
    var text        []byte
...
func (d *App01pgCustomer) JsonUnmarshal(text []byte) error {
	var err         error
...
func (s *App01pgCustomer) Key() Key {
    var k       Key
...
func (s *App01pgCustomer) KeysToValue() string {
    var wrk string
...
func (s *App01pgCustomer) ListOutput() string {
...
func (s *App01pgCustomer) Request2Struct(r *http.Request) error {
    var err         error
    var str         string
...
// SetKeysFromStrings creates a URL Value map from the table's key(s). The slice
// is in field order within the struct, not sorted by field name.
func (s *App01pgCustomer) SetKeysFromStrings(strs []string) error {
...
func (s *App01pgCustomer) TestData(i int) {
    var chr     rune
var i64     int64
var f64     float64
var str     string
...
func (s *App01pgCustomer) ToString(TitledName string) string {
    var str     string
...
func (s *App01pgCustomer) ToStrings() []string {
    var strs    []string
    var str     string
...
// NewApp01pgCustomer creates a new empty struct.
func NewApp01pgCustomer() *App01pgCustomer {
    return &App01pgCustomer{}
}

func NewApp01pgCustomers() *App01pgCustomers {
    return &App01pgCustomers{}
}
