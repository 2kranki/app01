...
def         perform_actions():
    ...
    if args.verbose > 0:
        ...
        print('*****************************************')
        print()
    rc = util.do_cmd("./jenkins/deploy/deploy.py", 'app01ma')
    if not rc == 0:
        print("Error: Deploying failed for: 'app01ma'!\n")
        return 4
    rc = util.do_cmd("./jenkins/deploy/deploy.py", 'app01ms')
    if not rc == 0:
        print("Error: Deploying failed for: 'app01ms'!\n")
        return 4
    rc = util.do_cmd("./jenkins/deploy/deploy.py", 'app01my')
    if not rc == 0:
        print("Error: Deploying failed for: 'app01my'!\n")
        return 4
    rc = util.do_cmd("./jenkins/deploy/deploy.py", 'app01pg')
    if not rc == 0:
        print("Error: Deploying failed for: 'app01pg'!\n")
        return 4
    rc = util.do_cmd("./jenkins/deploy/deploy.py", 'app01sq')
    if not rc == 0:
