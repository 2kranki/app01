...
def         perform_actions():
    ...
    rc = util.do_cmd("./jenkins/test/test.py", 'app01ma')
    if not rc == 0:
        print("Error: Testing failed for: 'app01ma'!\n")
        return 4
    rc = util.do_cmd("./jenkins/test/test.py", 'app01ms')
    if not rc == 0:
        print("Error: Testing failed for: 'app01ms'!\n")
        return 4
    rc = util.do_cmd("./jenkins/test/test.py", 'app01my')
    if not rc == 0:
        print("Error: Testing failed for: 'app01my'!\n")
        return 4
    rc = util.do_cmd("./jenkins/test/test.py", 'app01pg')
    if not rc == 0:
        print("Error: Testing failed for: 'app01pg'!\n")
        return 4
    rc = util.do_cmd("./jenkins/test/test.py", 'app01sq')
    if not rc == 0:
        print("Error: Testing failed for: 'app01sq'!\n")
        return 4
...
def main_cli(listArgV=None):
    ...
    if args.fDebug:
        ...
        print("\tSet util Debug : true")
    if args.fForce:
        util.force_flag = True
        print("\tSet util Force : true")
    if args.iVerbose > 0:
        util.trace_flag = True
        print("\tSet util Trace : true")
...
if __name__ == '__main__':
    ...
    if args.iVerbose or args.fDebug:
        if rc == 0:
