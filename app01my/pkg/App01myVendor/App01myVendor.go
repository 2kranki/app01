...
//  Struct and Methods for App01myVendor

// Generated: Wed Nov 20, 2019 16:06


package App01myVendor
...
type App01myVendorDbRowDeleter interface {
    // RowDelete deletes the row with keys from the provided record, rcd.
    RowDelete(rcd *App01myVendor) error
}

type App01myVendorDbRowFinder interface {
    ...
    RowFind(rcd *App01myVendor) error
}

type App01myVendorDbRowFirster interface {
    // RowFirst returns the first row in the table, Vendor.
    // If there are no rows in the table, then a blank/null record is returned
    // without error.
    RowFirst(rcd *App01myVendor) error
}

type App01myVendorDbRowInserter interface {
    RowInsert(rcd *App01myVendor) error
}

type App01myVendorDbRowLaster interface {
    // RowLast returns the last row in the table, Vendor.
    // If there are no rows in the table, then a blank/null record is returned
    // without error.
    RowLast(rcd *App01myVendor) error
}

type App01myVendorDbRowNexter interface {
    ...
    RowNext(rcd *App01myVendor) error
}

type App01myVendorDbRowPager interface {
    ...
    RowPage(offset int, limit int) ([]App01myVendor, error)
}

type App01myVendorDbRowPrever interface {
    RowPrev(rcd *App01myVendor) error
}

type App01myVendorDbRowUpdater interface {
    RowUpdate(rcd *App01myVendor) error
}

type App01myVendorDbTableCounter interface {
    TableCount() (int, error)
}

type App01myVendorDbTableCreater interface {
    TableCreate() error
}

type App01myVendorDbTableDeleter interface {
    TableDelete() error
}

type App01myVendorDbTableScanner interface {
    // TableScan reads all the rows in the table applying a function to each of
    // them.
    TableScan(apply func (rcd App01myVendor) error) error
}
...
type App01myVendors []*App01myVendor

type Key struct {
	Id	int64
}

type App01myVendorIndex map[Key]*App01myVendor
...
func (s *App01myVendor) Compare(r *App01myVendor) int {
...
func (s *App01myVendor) CompareKeys(r *App01myVendor) int {
    // Accumulate the key value(s) in KeyNum order.
    // Field: Id
            if s.Id != r.Id {
                if s.Id < r.Id {
...
func (s *App01myVendor) FieldsToValue() string {
    var wrk string
...
func (d *App01myVendor) JsonMarshal() ([]byte, error) {
	var err         error
    var text        []byte
...
func (d *App01myVendor) JsonUnmarshal(text []byte) error {
	var err         error
...
func (s *App01myVendor) KeysToValue() string {
    var wrk string
...
func (s *App01myVendor) ListOutput() string {
...
func (s *App01myVendor) Request2Struct(r *http.Request) error {
    var err         error
    var str         string
...
// SetKeysFromStrings creates a URL Value map from the table's key(s). The slice
// is in field order within the struct, not sorted by field name.
func (s *App01myVendor) SetKeysFromStrings(strs []string) error {
...
func (s *App01myVendor) TestData(i int) {
    var chr     rune
var i64     int64
var f64     float64
var str     string
...
func (s *App01myVendor) ToString(TitledName string) string {
    var str     string
...
func (s *App01myVendor) ToStrings() []string {
    var strs    []string
    var str     string
...
// NewApp01myVendor creates a new empty struct.
func NewApp01myVendor() *App01myVendor {
    return &App01myVendor{}
}

func NewApp01myVendors() *App01myVendors {
    return &App01myVendors{}
}
