...
func main() {
	...
	wrk = os.Getenv("APP01MY_HTTPPORT")
	if len(wrk) > 0 {
		http_port = wrk
	}
	wrk = os.Getenv("APP01MY_HTTPSERVER")
	if len(wrk) > 0 {
		http_srvr = wrk
	}
	...
	wrk = os.Getenv("APP01MY_DBPW")
	if len(wrk) > 0 {
		db_pw = wrk
	}
	wrk = os.Getenv("APP01MY_DBPORT")
	if len(wrk) > 0 {
		db_port = wrk
	}
	wrk = os.Getenv("APP01MY_DBSERVER")
	if len(wrk) > 0 {
		db_srvr = wrk
	}
	wrk = os.Getenv("APP01MY_DBUSER")
	if len(wrk) > 0 {
		db_user = wrk
	}
	wrk = os.Getenv("APP01MY_DBNAME")
	if len(wrk) > 0 {
		db_name = wrk
	}
