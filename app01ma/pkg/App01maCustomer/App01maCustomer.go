...
//  Struct and Methods for App01maCustomer

// Generated: Mon Jan  6, 2020 09:54

package App01maCustomer
...
type App01maCustomerDbRowDeleter interface {
	// RowDelete deletes the row with keys from the provided record, rcd.
	RowDelete(rcd *App01maCustomer) error
}

type App01maCustomerDbRowFinder interface {
	...
	RowFind(rcd *App01maCustomer) error
}

type App01maCustomerDbRowFirster interface {
	...
	// without error.
	RowFirst(rcd *App01maCustomer) error
}

type App01maCustomerDbRowInserter interface {
	RowInsert(rcd *App01maCustomer) error
}

type App01maCustomerDbRowLaster interface {
	// RowLast returns the last row in the table, Customer.
	// If there are no rows in the table, then a blank/null record is returned
	// without error.
	RowLast(rcd *App01maCustomer) error
}

type App01maCustomerDbRowNexter interface {
	...
	RowNext(rcd *App01maCustomer) error
}

type App01maCustomerDbRowPager interface {
	...
	RowPage(offset int, limit int) ([]App01maCustomer, error)
}

type App01maCustomerDbRowPrever interface {
	RowPrev(rcd *App01maCustomer) error
}

type App01maCustomerDbRowUpdater interface {
	RowUpdate(rcd *App01maCustomer) error
}

type App01maCustomerDbTableCounter interface {
	TableCount() (int, error)
}

type App01maCustomerDbTableCreater interface {
	TableCreate() error
}

type App01maCustomerDbTableDeleter interface {
	TableDelete() error
}

type App01maCustomerDbTableScanner interface {
	// TableScan reads all the rows in the table applying a function to each of
	// them.
	TableScan(apply func(rcd App01maCustomer) error) error
}
...
type App01maCustomers []*App01maCustomer

type Key struct {
	Num int64
}

type App01maCustomerIndex map[Key]*App01maCustomer
...
func (s *App01maCustomer) Compare(r *App01maCustomer) int {
...
func (s *App01maCustomer) CompareKeys(r *App01maCustomer) int {
	// Accumulate the key value(s) in KeyNum order.
	// Field: Num
	if s.Num != r.Num {
		if s.Num < r.Num {
...
func (s *App01maCustomer) FieldsToValue() string {
	var wrk string
...
func (d *App01maCustomer) JsonMarshal() ([]byte, error) {
	var err error
	var text []byte
...
func (d *App01maCustomer) JsonUnmarshal(text []byte) error {
	var err error
...
func (s *App01maCustomer) Key() Key {
	var k Key
...
func (s *App01maCustomer) KeysToValue() string {
	var wrk string
...
func (s *App01maCustomer) ListOutput() string {
...
func (s *App01maCustomer) Request2Struct(r *http.Request) error {
	var err error
	var str string
...
// SetKeysFromStrings creates a URL Value map from the table's key(s). The slice
// is in field order within the struct, not sorted by field name.
func (s *App01maCustomer) SetKeysFromStrings(strs []string) error {
...
func (s *App01maCustomer) TestData(i int) {
	var chr rune
	var i64 int64
	var f64 float64
	var str string
...
func (s *App01maCustomer) ToString(TitledName string) string {
	var str string
...
func (s *App01maCustomer) ToStrings() []string {
	var strs []string
	var str string
...
// NewApp01maCustomer creates a new empty struct.
func NewApp01maCustomer() *App01maCustomer {
	return &App01maCustomer{}
}

func NewApp01maCustomers() *App01maCustomers {
	return &App01maCustomers{}
}
