...
type App01msVendorDbRowDeleter interface {
    // RowDelete deletes the row with keys from the provided record, rcd.
    RowDelete(rcd *App01msVendor) error
}

type App01msVendorDbRowFinder interface {
    ...
    RowFind(rcd *App01msVendor) error
}

type App01msVendorDbRowFirster interface {
    // RowFirst returns the first row in the table, Vendor.
    // If there are no rows in the table, then a blank/null record is returned
    // without error.
    RowFirst(rcd *App01msVendor) error
}

type App01msVendorDbRowInserter interface {
    RowInsert(rcd *App01msVendor) error
}

type App01msVendorDbRowLaster interface {
    // RowLast returns the last row in the table, Vendor.
    // If there are no rows in the table, then a blank/null record is returned
    // without error.
    RowLast(rcd *App01msVendor) error
}

type App01msVendorDbRowNexter interface {
    ...
    RowNext(rcd *App01msVendor) error
}

type App01msVendorDbRowPager interface {
    ...
    RowPage(offset int, limit int) ([]App01msVendor, error)
}

type App01msVendorDbRowPrever interface {
    RowPrev(rcd *App01msVendor) error
}

type App01msVendorDbRowUpdater interface {
    RowUpdate(rcd *App01msVendor) error
}

type App01msVendorDbTableCounter interface {
    TableCount() (int, error)
}

type App01msVendorDbTableCreater interface {
    TableCreate() error
}

type App01msVendorDbTableDeleter interface {
    TableDelete() error
}

type App01msVendorDbTableScanner interface {
    // TableScan reads all the rows in the table applying a function to each of
    // them.
    TableScan(apply func (rcd App01msVendor) error) error
}
...
type App01msVendors []*App01msVendor

type Key struct {
	Id	int64
}

type App01msVendorIndex map[Key]*App01msVendor
...
func (s *App01msVendor) Compare(r *App01msVendor) int {
...
func (s *App01msVendor) CompareKeys(r *App01msVendor) int {
    // Accumulate the key value(s) in KeyNum order.
    // Field: Id
            if s.Id != r.Id {
                if s.Id < r.Id {
...
func (s *App01msVendor) FieldsToValue() string {
    var wrk string
...
func (d *App01msVendor) JsonMarshal() ([]byte, error) {
	var err         error
    var text        []byte
...
func (d *App01msVendor) JsonUnmarshal(text []byte) error {
	var err         error
...
func (s *App01msVendor) KeysToValue() string {
    var wrk string
...
func (s *App01msVendor) ListOutput() string {
...
func (s *App01msVendor) Request2Struct(r *http.Request) error {
    var err         error
    var str         string
...
// SetKeysFromStrings creates a URL Value map from the table's key(s). The slice
// is in field order within the struct, not sorted by field name.
func (s *App01msVendor) SetKeysFromStrings(strs []string) error {
...
func (s *App01msVendor) TestData(i int) {
    var chr     rune
var i64     int64
var f64     float64
var str     string
...
func (s *App01msVendor) ToString(TitledName string) string {
    var str     string
...
func (s *App01msVendor) ToStrings() []string {
    var strs    []string
    var str     string
...
// NewApp01msVendor creates a new empty struct.
func NewApp01msVendor() *App01msVendor {
    return &App01msVendor{}
}

func NewApp01msVendors() *App01msVendors {
    return &App01msVendors{}
}
