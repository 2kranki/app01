...
type App01msCustomerDbRowDeleter interface {
    // RowDelete deletes the row with keys from the provided record, rcd.
    RowDelete(rcd *App01msCustomer) error
}

type App01msCustomerDbRowFinder interface {
    ...
    RowFind(rcd *App01msCustomer) error
}

type App01msCustomerDbRowFirster interface {
    ...
    // without error.
    RowFirst(rcd *App01msCustomer) error
}

type App01msCustomerDbRowInserter interface {
    RowInsert(rcd *App01msCustomer) error
}

type App01msCustomerDbRowLaster interface {
    // RowLast returns the last row in the table, Customer.
    // If there are no rows in the table, then a blank/null record is returned
    // without error.
    RowLast(rcd *App01msCustomer) error
}

type App01msCustomerDbRowNexter interface {
    ...
    RowNext(rcd *App01msCustomer) error
}

type App01msCustomerDbRowPager interface {
    ...
    RowPage(offset int, limit int) ([]App01msCustomer, error)
}

type App01msCustomerDbRowPrever interface {
    RowPrev(rcd *App01msCustomer) error
}

type App01msCustomerDbRowUpdater interface {
    RowUpdate(rcd *App01msCustomer) error
}

type App01msCustomerDbTableCounter interface {
    TableCount() (int, error)
}

type App01msCustomerDbTableCreater interface {
    TableCreate() error
}

type App01msCustomerDbTableDeleter interface {
    TableDelete() error
}

type App01msCustomerDbTableScanner interface {
    // TableScan reads all the rows in the table applying a function to each of
    // them.
    TableScan(apply func (rcd App01msCustomer) error) error
}
...
func (s *App01msCustomer) Compare(r *App01msCustomer) int {
...
func (s *App01msCustomer) CompareKeys(r *App01msCustomer) int {
    // Accumulate the key value(s) in KeyNum order.
    // Field: Num
            if s.Num != r.Num {
                if s.Num < r.Num {
...
func (s *App01msCustomer) FieldsToValue() string {
    var wrk string
...
func (s *App01msCustomer) KeysToValue() string {
    var wrk string
...
func (s *App01msCustomer) ListOutput() string {
...
func (s *App01msCustomer) Request2Struct(r *http.Request) error {
    var err         error
    var str         string
...
// SetKeysFromStrings creates a URL Value map from the table's key(s). The slice
// is in field order within the struct, not sorted by field name.
func (s *App01msCustomer) SetKeysFromStrings(strs []string) error {
...
func (s *App01msCustomer) TestData(i int) {
    var chr     rune
var i64     int64
var f64     float64
var str     string
...
func (s *App01msCustomer) ToString(TitledName string) string {
    var str     string
...
func (s *App01msCustomer) ToStrings() []string {
    var strs    []string
    var str     string
...
// NewApp01msCustomer creates a new empty struct.
